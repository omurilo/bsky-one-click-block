document.addEventListener('DOMContentLoaded', () => {
  ...
  for (const btn of postDropdownBtn.values()) {
    if (btn.offsetParent !== null) {
      let username = "";
      const root = btn.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.dataset;
      if (root.testid.includes('feedItem-by-')) {
      username = root.testid.replace('feedItem-by-', '');
    }
...
document.addEventListener('DOMNodeInserted', debounce((e) => {
  if (window.pause_event !== true) {
    ...
    console.log('// check if new post dropdown node inserted')
    let username = '';
    const root = e.target.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.dataset;
    if (root.testid.includes('feedItem-by-')) {
      username = root.testid.replace('feedItem-by-', '');
    }
...
function createBlockUserBtn(baseBtn, username) {
  ...
  icon.setAttribute('class', 'fas fa-ban')
